...
exports.setEmployeeRoleAndTenant = exports.onUserCreated = void 0;
...
exports.onUserCreated = functions.auth.user().onCreate(async (user) => {
    ...
    try {
        ...
        if (adminsDoc.exists) {
            const adminsList = adminsDoc.data()?.admins || [];
 ...
 */
exports.setEmployeeRoleAndTenant = functions.https.onCall(async (data, context) => {
    ...
    if (!["cashier", "manager", "admin"].includes(normalizedRole)) {
        throw new functions.https.HttpsError("invalid-argument", "Role must be one of: 'cashier', 'manager', 'admin'.");
    }
    try {
